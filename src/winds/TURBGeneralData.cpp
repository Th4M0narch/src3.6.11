...
TURBGeneralData::TURBGeneralData(const WINDSInputData *WID, WINDSGeneralData *WGDin)
{
  ...
  if (WID->turbParams->sigConst) {
    ...
    // 打印输出计算得到的常数值
    std::cout << "sigUConst: " << sigUConst << std::endl;
    std::cout << "sigVConst: " << sigVConst << std::endl;
    std::cout << "sigWConst: " << sigWConst << std::endl;
  }
...
// bldgH_mean = 10;
  ...
  std::cout << "Calculated roughness length (z0): " << z0d << " meters" << std::endl;
