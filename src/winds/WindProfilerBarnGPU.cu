...
void BarnesSchemeStep2(float *d_wm, float *d_sum_wm, float *d_sum_wu, float *d_sum_wv, float *d_u0, float *d_v0, float *d_w0, float *d_u_prof, float *d_v_prof, int *d_site_id, int *d_terrain_face_id, int *d_k_mod, int num_sites, int nx, int ny, int nz, float asl_percent, float abl_height, float *d_z, float *d_surf_layer_height) {
...
void BarnesSchemeStep3(float *d_site_xcoord, float *d_site_ycoord, float *d_x, float *d_y, float *d_dxx, float *d_dyy, int *d_iwork, int *d_jwork, float *d_u12, float *d_u34, float *d_v12, float *d_v34, float *d_u0_int, float *d_v0_int, float *d_u0, float *d_v0, float *d_u_prof, float *d_v_prof, int *d_site_id, int *d_terrain_face_id, int num_sites, float dx, float dy, int nx, int ny, int nz) {
...
void BarnesSchemeStep4(float *d_wm, float *d_sum_wm, float *d_sum_wu, float *d_sum_wv, float *d_u0, float *d_v0, float *d_w0, float *d_u_prof, float *d_v_prof, float *d_u0_int, float *d_v0_int, int *d_site_id, int *d_terrain_face_id, int *d_k_mod, int num_sites, int nx, int ny, int nz, float asl_percent, float abl_height, float *d_z, float *d_surf_layer_height) {
    dim3 numberOfThreadsPerBlock1(BLOCKSIZE, 1, 1);
    dim3 numberOfBlocks1(ceil((nx * ny) / (float)(BLOCKSIZE)), 1, 1);
...
void WindProfilerBarnGPU::BarnesInterpolationGPU(const WINDSInputData *WID, WINDSGeneralData *WGD)
{
    ...
    float rc_sum = 0.0;
    for (int i = 0; i < num_sites; i++) {
        float rc_val = 1000000.0;
        for (int ii = 0; ii < num_sites; ii++) {
            if (ii != i) {
                float xc = site_xcoord[ii] - site_xcoord[i];
                float yc = site_ycoord[ii] - site_ycoord[i];
                float rc = sqrt(pow(xc, 2.0) + pow(yc, 2.0));
                if (rc < rc_val) {
                    rc_val = rc;
                }
            }
    ...
    float dn = rc_sum / num_sites;
    float lambda = 5.052 * pow((2 * dn / M_PI), 2.0);
    float s_gamma = 0.2;
    ...
    // 计算和同步步骤
    dim3 blocksPerGrid((num_sites * WGD->nx * WGD->ny + BLOCKSIZE - 1) / BLOCKSIZE);
    dim3 threadsPerBlock(BLOCKSIZE);

    // Step 1: 计算 wm
    Calculatewm<<<blocksPerGrid, threadsPerBlock>>>(d_wm, d_wms, d_sum_wm, d_site_xcoord, d_site_ycoord, d_x, d_y, lambda, s_gamma, num_sites, WGD->nx, WGD->ny, WGD->nz);
    cudaCheck(cudaDeviceSynchronize());

    // Step 2: 计算初始风场
    CalculateInitialWind<<<blocksPerGrid, threadsPerBlock>>>(d_wm, d_sum_wm, d_sum_wu, d_sum_wv, d_u0, d_v0, d_w0, d_u_prof, d_v_prof, d_site_id, d_terrain_face_id, d_k_mod, num_sites, WGD->nx, WGD->ny, WGD->nz, asl_percent, abl_height, d_z, d_surf_layer_height);
    cudaCheck(cudaDeviceSynchronize());

    // Step 3: 初始化风场
    CalculateInit<<<blocksPerGrid, threadsPerBlock>>>(d_site_xcoord, d_site_ycoord, d_x, d_y, d_dxx, d_dyy, d_iwork, d_jwork, d_u12, d_u34, d_v12, d_v34, d_u0_int, d_v0_int, d_u0, d_v0, d_u_prof, d_v_prof, d_site_id, d_terrain_face_id, num_sites, WGD->dx, WGD->dy, WGD->nx, WGD->ny, WGD->nz);
    cudaCheck(cudaDeviceSynchronize());

    // Step 4: 修正初始风场
    CorrectInitialWind<<<blocksPerGrid, threadsPerBlock>>>(d_wm, d_sum_wm, d_sum_wu, d_sum_wv, d_u0, d_v0, d_w0, d_u_prof, d_v_prof, d_u0_int, d_v0_int, d_site_id, d_terrain_face_id, d_k_mod, num_sites, WGD->nx, WGD->ny, WGD->nz, asl_percent, abl_height, d_z, d_surf_layer_height);
    cudaCheck(cudaDeviceSynchronize());
